...
TCP logging. This is useful when a route is configured for SSL passthrough.
...
1. Cluster admin wants to set custom log format for HTTP, HTTPS, and TCP for all the routes of an Ingress Controller.
   Cluster admin edits/create the CR of the Ingress Controller having Spec as follows -
```yaml
apiVersion: operator.openshift.io/v1
kind: IngressController
metadata:
...
This proposal does not include any functional differences for
OpenShift Kubernetes Engine.
